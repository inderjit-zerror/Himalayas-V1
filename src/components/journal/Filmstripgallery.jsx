  ...
  return (
    <div
      className={`relative shrink-0  flex flex-col group ${
        isLarge
          ? `w-[220px] sm:w-[320px] ${ed === 1 ? "max-w:w-full" : "md:w-1/4"}`
          : `w-[180px] sm:w-[260px] ${ed === 1 ? "max-w:w-full" : "md:w-1/4"}`
      }`}
    >
      {/* 
        Moved the fixed heights HERE to the image container. 
        This allows the text to naturally sit below the image without being clipped.
      */}
      <div
        className={`relative w-full overflow-hidden bg-gray-900 ${
          isLarge
            ? "h-[310px] sm:h-[450px] md:h-[600px]"
            : "h-[250px] sm:h-[360px] md:h-[480px]"
        }`}
      >
        <img
          src={src}
          alt={alt}
          // Added group-hover:scale-110 and transition to scale the image slightly on hover
          className="w-full h-full object-cover grayscale-[20%] contrast-125 transition-transform duration-500 ease-out group-hover:scale-110 pointer-events-none"
          draggable="false"
        />
      ...
      {/* 
        Fixed: group-hover:opacity-100 instead of opacity-1 
        Added: transition-opacity duration-300 for a smooth fade
      */}
      <div className="mt-3 sm:mt-4 text-xs sm:text-sm leading-relaxed text-gray-800 opacity-0 group-hover:opacity-100 transition-opacity duration-300 ease-in-out">
        <p>
          <span className="font-semibold">Subject:</span> Itsuki Takahashi
        </p>
        <p>
          <span className="font-semibold">Project: </span> Feature in Yaraeleon
          Mag
        </p>
      </div>
    </div>
