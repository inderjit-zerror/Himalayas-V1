...
export default function ExpeditionPage() {
    ...
    const glanceData = [
        ...
        { label: 'Difficulty:', value: 'Experienced Riders (DH to confirm wording)' },
        { label: 'Private Departures:', value: 'Available on request' },
        { label: 'Upcoming Departure:', value: '20 Oct 2026' },
    ];

    return (
        <div className="min-h-screen bg-white text-black font-sans selection:bg-gray-800 selection:text-white pt-[10vh] px-10">
            {/* Navigation Header */}
            <header className="w-full pt-10 px-6 md:px-12 flex justify-center lg:justify-center z-20 relative">
                <div className="flex flex-col md:flex-row shadow-sm">
                    ...
                    {/* Active Tab with Dropdown - Added ref={dropdownRef} here */}
                    <div className="relative w-full md:w-[260px]" ref={dropdownRef}>
                        <button
                            onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                            className="w-full bg-[#333333] text-white text-xs font-semibold tracking-[0.15em] px-6 py-4 flex justify-between z-999 items-center outline-none transition-colors hover:bg-black"
                        >
                            HIMALAYAN JOURNEYS
                            <span className={`text-lg leading-none ml-2 transition-transform duration-200 ${isDropdownOpen ? 'rotate-90' : ''}`}>
                                &rsaquo;
                            </span>
                        </button>
                        ...
                        {isDropdownOpen && (
                            <div className="absolute top-full left-0 w-full border border-t-0 border-gray-400 bg-white pt-6 pb-8 pl-10 pr-6 z-10 shadow-lg md:shadow-none origin-top animate-in fade-in slide-in-from-top-2">
                    ...
                    {/* Inactive Tab */}
                    <button className="w-full md:w-[260px] bg-white text-gray-500 text-xs font-semibold tracking-[0.15em] px-6 py-4 border border-gray-400 md:border-l-0 flex justify-between items-center hover:bg-gray-50 transition-colors outline-none">
                        BEYOND HIMALAYAS <span className="text-lg leading-none ml-2">&rsaquo;</span>
                    </button>
                </div>
            </header>
